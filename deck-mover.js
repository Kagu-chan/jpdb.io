...
(function() {
    ...
    const init = () => {
        Array.from(document.querySelectorAll('.vocabulary-list .dropdown .dropdown-content>ul')).forEach((node) => {
            const cn = Array.from(node.childNodes);
            const targetNode = cn.find(({ childNodes }) => childNodes[0].innerHTML?.startsWith('Edit'));
            const deckSelectionQuery = cn.find(({ childNodes }) => childNodes[0].innerHTML === 'Add to a deck').childNodes[0]?.href?.split('?')[1];
