...
class Scope {
    ...
    constructor(options, ...variants) {
        const variant = variants.find(({ on }) => typeof on === 'string' ? on === this.path : on.test(this.path));
        if (!variant) return;
        assign(options, variant);

        this.options = options;
        this.nodeContainer = new NodeContainer(this.options, Node.getNodes(this.options.select, undefined, 1));
        this.active = true;
    }

    add(name, text, callback) {
        this.actions[name] = this.actions[name] || new NodeAction(this.nodeContainer, name, text, callback);
...
class ActionsManager {
    ...
    scopes = {
        yourDecks: new Scope({ id: 'your-decks', style: { opacity: .5, 'margin-left': '12px', 'font-size': 'small', cursor: 'pointer' } }, { on: '/learn', select: 'h4#deck_list' }, { on: '/deck-list', select: '.container > h4' }),
        deckActions: new Scope({ id: 'deck-actions' }, { on: '/deck-list', select: '.container > div:first-of-type' }),
    };
