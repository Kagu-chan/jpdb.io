...
const Deck = class {
    static Coverage = class {
        constructor(node) {
            ...
            const info = vocab ? undefined : Node.getChildNodes(this.nodes.node, '.deck-body > div', undefined, 1);
        ...
        setWordData() {
            const [words, wordsMax] = this.nodes.vocab.childNodes[0].childNodes[1].innerHTML.split(Deck.WORD_SEPARATOR).map((e) => Number(e));
...
const Script = (...activeAt) => class {
    static activeAt = activeAt;
    static path = window.location.pathname;
    static isActive() {
        return !!this.activeAt.find((expression) => typeof expression === 'string' ? expression === this.path : expression.test(this.path));
    }
